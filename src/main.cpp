...
uint256 hashGenesisBlock("0xfa514ed4b7dcd1c2e089154400cb3c2bf15b7885075b00fa358371b9663027ad");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryptoapples: starting difficulty is 1 / 2^12
...
static const int64 nDiffChangeTargetpatch = 86400;
...
//
static const int64 patchBlockDuration = 5040;
...
static const int64 patchNumberOfBlocks = (patchBlockDuration / 20)-245;
...
static const int64 npatchAmountofblocks = patchBlockDuration / 105;
...
static const int64 patchValueOfBlocks = (npatchAmountofblocks * nDiffChangeTargetpatch)-647200;
...
static const int64 npatchNumberOfBlocks = (nDiffChangeTargetpatch / 200)-426;
...
static const int64 nDiffChangeTarget = 86400;
...
int64 GetCRASubsidy(int nHeight) {
    ...
    //for each 3 days that has passed, decrease reward by 1%
...
int64 static GetBlockValue(int nHeight, int64 nFees) {
    int64 nSubsidy = COIN;
        // this is official premine 1,2% for bounties and giveaways
    if(nHeight < 2) {
        nSubsidy = 60540 * COIN;
        ...
        //this is starting reward for first 14 days after launch
            nSubsidy = 32 * COIN;

    } else if(nHeight < nDiffChangeTarget) {
        //this is pre-patch, reward is 16.
        nSubsidy = 16 * COIN;
    ...
    nSubsidy = patchValueOfBlocks*COIN; }
...
static const int64 nTargetTimespan = 2 * 60;	// Cryptoapples: 2 min
static const int64 nTargetSpacing = 120;						// Cryptoapples: 2 min
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 2 * 60; // 2 min
static const int64 nTargetSpacingRe = 120; // Cryptoapples: 2 min
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlock *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
	...
	for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
		...
		EventHorizonDeviation			= 1 + (0.7084 * pow((double(PastBlocksMass)/double(120)), -1.228));
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast, const CBlock *pblock)
{
	...
	int64				PastSecondsMin				= TimeDaySeconds * 0.275;
	int64				PastSecondsMax				= TimeDaySeconds * 0.945;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Cryptoapples miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Cryptoapples", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7c6a4801f129ea31ff8fda7db50e2f338c91bab0a1cbad74c59d868b24b47f36");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Feb 14, mi z vyvoje noveho coinu prasknul curak";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391425725;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 285555;

        if (fTestNet)
        {
            block.nTime    = 1391373050;
            block.nNonce   = 436345;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x49558812ca1c84c231cae23b31a3246a281e678eaf4437227d89cc33d4ccaa47"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cryptoapples: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Cryptoapplesd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
